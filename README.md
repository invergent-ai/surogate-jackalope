...
A native desktop app for the [**Surogate**](https://github.com/invergent-ai/surogate) training engine.
...
| | |
|---|---|
| 📈 **Monitor** | Live loss curve, per-GPU temp/power/util/memory meters, step & epoch progress, throughput, and a streaming log — in real time. |
| 🚀 **Launch** | **SFT**, **GRPO**, and **RULER** (GRPO + an LLM judge). Choose model, dataset, precision recipe and learning rate inline. **Click GPUs to select** — no typing indices. |
| ☁️ **Train anywhere** | **Local** GPUs · remote **SSH** box · serverless **Modal** · your own cloud via **dstack**. Secrets stay in each tool's own store — never in Jackalope. |
| 🔎 **Models & Datasets** | Search the Hugging Face Hub right inside the app and line up a run. |
| 🖥️ **GPUs · Runs · Logs · Files** | Inspect local devices, browse past runs, stream logs, and pull trained artifacts back to disk. |
...
> To **run training** you also need the **Surogate** engine reachable (locally, over
> SSH, or in the cloud):
> ```bash
...
Requirements: **Node ≥ 20**, **Rust** (stable). Linux also needs WebKitGTK / GTK dev
packages (`webkit2gtk-4.1`, `libappindicator3`, `librsvg2`).
...
Jackalope doesn't reimplement training — it **drives Surogate**. A small **Rust**
core tails the JSONL metrics feed Surogate writes (`report_to: [surogate]`) to power
the live dashboard, and shells out to the `surogate` CLI to start runs (locally, in
a detached SSH/tmux session, a Modal sandbox, or via `dstack apply`). The **React**
UI is just the view; everything privileged happens in Rust.
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/invergent-ai/surogate-jackalope/main/assets/surogate-logo-white.svg" />
  <img alt="Surogate" width="200" src="https://raw.githubusercontent.com/invergent-ai/surogate-jackalope/main/assets/surogate-logo-black.svg" />
</picture>
...
| 💎 **Precision** | **BF16** · native **FP8** (E4M3/E5M2) · **NVFP4** 4-bit on Blackwell (B200/B300, RTX 50xx) |
| 🤖 **RL** | **GRPO** with deterministic [RL environments](https://docs.surogate.ai/guides/rl-environments) (+ RULER LLM-judge) |
| 🖥️ **Scale** | Native [multi-GPU](https://docs.surogate.ai/guides/multi-gpu) & [multi-node](https://docs.surogate.ai/guides/multi-node) (Ray DDP) |
| ⚖️ **Offload** | Smart CPU offloading — fine-tune at native bf16, making QLoRA optional |
| 🎨 **MoE** | Expert parallelism, load balancing, imbalance detection |
| 🧬 **Adaptive** | Auto phase detection, early stopping, dynamic LR & token budgeting |

Runs on every modern NVIDIA GPU (sm80 → sm120).
...
Learn more at **[surogate.ai](https://surogate.ai)** and the **[docs](https://docs.surogate.ai)**.
